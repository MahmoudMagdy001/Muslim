...
class _HadithBooksViewContent extends StatefulWidget {
  const _HadithBooksViewContent();

  @override
  State<_HadithBooksViewContent> createState() =>
      _HadithBooksViewContentState();
}
...
class _HadithBooksViewContentState extends State<_HadithBooksViewContent> {
  ...
  List<HadithBookEntity> _filterBooks(
    List<HadithBookEntity> books,
    String searchText,
    String languageCode,
  ) {
    if (searchText.trim().isEmpty) return books;
                ...
                child: BlocBuilder<HadithBooksCubit, HadithBooksState>(
                  builder: (context, state) {
                    if (state.status == HadithBooksStatus.loading ||
                      ...
                      return CustomLoadingIndicator(text: localization.loading);
                    }
                          ...
                          itemBuilder: (context, index) {
                            if (index == 0) {
                              return HadithOfTheDayCard(
                                status: state.randomHadithStatus,
                                data: state.randomHadithData,
                              );
                            }
